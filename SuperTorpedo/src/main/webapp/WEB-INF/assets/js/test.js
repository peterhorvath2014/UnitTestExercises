app.directive('donutChart', function() {
	...
	function link(scope, element) {
		...
		var arc = d3.svg.arc().outerRadius(min / 2 * 0.9).innerRadius(
				min / 2 * 0.5);
		...
		svg.on('mousedown', function(d) {
			// yo angular, the code in this callback might make a change to the
			// scope!
		...
		var arcs = svg.selectAll('path.arc').data(pie(scope.data)).enter()
				.append('path').attr('class', 'arc').style('stroke', 'white')
		...
		scope.$watch('data', function(newData, oldData) {
			...
			arcs.enter().append('path').style('stroke', 'white').attr('class',
			...
			arcs.filter(function(d) {
				return d.data === 0
			}).transition().duration(duration).each(function(d) {
